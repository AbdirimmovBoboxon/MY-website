...
(() => {
  ...
  const initSaturn3D = ({ animate }) => {
    ...
    camera.position.set(0.0, 0.0, 6.2);
    ...
    // Load real Saturn texture from local images
    const loader = new THREE.TextureLoader();

    // Try local image first, fallback to assets folder
    const tryLoad = (urls, onLoad) => {
      const attempt = (i) => {
        if (i >= urls.length) return;
        loader.load(urls[i], onLoad, undefined, () => attempt(i + 1));
      };
      attempt(0);
    };

    tryLoad(['images/fullsize/176saturn.jpg', 'assets/textures/saturn_color.jpg'], (tex) => {
        applyColorSpace(tex);
        tex.wrapS = THREE.RepeatWrapping;
        tex.wrapT = THREE.ClampToEdgeWrapping;
    ...
    tryLoad(['assets/textures/saturn_ring.png', 'images/fullsize/176saturn.jpg'],
      (tex) => {
        applyColorSpace(tex);
        tex.wrapS = THREE.ClampToEdgeWrapping;
        tex.wrapT = THREE.ClampToEdgeWrapping;
    ...
    const animateFrame = () => {
      ...
      // VERY slow rotation — cinematic
      planet.rotation.y += dt * 0.10;
      ringGroup.rotation.z += dt * 0.02;

      // Gentle float up/down at center
      saturn.position.y = Math.sin(t * 0.25) * 0.09;
      saturn.position.x = 0;

      // Micro-parallax on mouse
