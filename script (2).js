...
(() => {
  ...
  let scrollRaf = 0;
  let saturnScroll01 = 0; // 0..1 scroll progress through hero (used for cinematic zoom)
  const heroSection = document.getElementById('home');
  let heroHeight = Math.max(1, heroSection?.getBoundingClientRect?.().height || window.innerHeight || 1);

  const measureHero = () => {
    heroHeight = Math.max(1, heroSection?.getBoundingClientRect?.().height || window.innerHeight || 1);
  };
  ...
  const updateScrollParallax = () => {
    ...
    // Fade + zoom only while the hero is on screen (prevents Saturn distracting later sections)
    const denom = Math.max(1, heroHeight * 0.9);
    saturnScroll01 = Math.max(0, Math.min(1, y / denom));
    const alpha = prefersReducedMotion ? 1 : Math.max(0, Math.min(1, 1 - saturnScroll01 * 1.15));
    root.style.setProperty('--saturn-alpha', alpha.toFixed(3));
    scrollRaf = 0;
  };
  ...
  const initSaturn3D = ({ animate }) => {
    ...
    const camBase = new THREE.Vector3(0.86, 0.18, 6.85);
    const camTarget = camBase.clone();
    const camCurrent = camBase.clone();
    camera.position.copy(camCurrent);

    const lookAt = new THREE.Vector3(0.34, 0.02, 0);
    camera.lookAt(lookAt);

    const ambient = new THREE.AmbientLight(0xffffff, 0.1);
    scene.add(ambient);

    const hemi = new THREE.HemisphereLight(0x9bd3ff, 0x070815, 0.22);
    scene.add(hemi);

    const sun = new THREE.DirectionalLight(0xffd2a6, 3.0);
    sun.position.set(3.7, 2.45, 4.6);
    scene.add(sun);

    const rim = new THREE.DirectionalLight(0x7dd3fc, 0.9);
    rim.position.set(-4.7, 0.6, -2.8);
    scene.add(rim);
    ...
    saturn.rotation.set(baseRot.x, baseRot.y, baseRot.z);
    saturn.position.x = 0.34;
    scene.add(saturn);
    ...
    const createRingTexture = () => {
      ...
      const inner = 1.62 / 2.78; // innerRadius / outerRadius
      const outer = 1.0;
    ...
    const planet = new THREE.Mesh(new THREE.SphereGeometry(1.34, 96, 96), planetMat);
    saturn.add(planet);

    // Cinematic fresnel rim glow (subtle "photographic" atmosphere)
    ...
    const atmo = new THREE.Mesh(new THREE.SphereGeometry(1.42, 64, 64), atmoMat);
    saturn.add(atmo);

    const ringGroup = new THREE.Group();
    ringGroup.rotation.set(THREE.MathUtils.degToRad(62), 0, THREE.MathUtils.degToRad(-18));
    saturn.add(ringGroup);

    const ringGeo = new THREE.RingGeometry(1.62, 2.78, 220);
    ...
    ringGlowMat.toneMapped = false;

    const ringGlow = new THREE.Mesh(ringGeo, ringGlowMat);
    ringGlow.scale.setScalar(1.02);
    ringGlow.rotation.z = ring.rotation.z;
    ringGroup.add(ringGlow);

    // Ring sparkle particles (additive points, very lightweight)
    const makeSparkleMap = () => {
      ...
      grad.addColorStop(0.18, 'rgba(255,226,168,0.65)');
      grad.addColorStop(0.55, 'rgba(255,226,168,0.18)');
    ...
    const sparkleMap = makeSparkleMap();

    const createRingParticles = ({ count, size, color, opacity }) => {
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        const rr = Math.sqrt(Math.random() * (outerR * outerR - innerR * innerR) + innerR * innerR);
      ...
      points.rotation.z = ring.rotation.z;
      return { points, mat };
    };

    const ringDust = createRingParticles({ count: 900, size: 0.055, color: 0xffe3b0, opacity: 0.18 });
    const ringSparkles = createRingParticles({ count: 240, size: 0.09, color: 0xffffff, opacity: 0.08 });
    ringGroup.add(ringDust.points);
    ringGroup.add(ringSparkles.points);

    // Replace procedural textures with real files (Planet Pixel Emporium) when present
    ...
    const loadWithFallback = (urls, onLoad) => {
      const tryLoad = (index) => {
    ...
    loadWithFallback(['../images/fullsize/176saturn.jpg', 'assets/textures/saturn.jpg', 'assets/textures/saturn_color.jpg'], (tex) => {
    ...
    loadWithFallback(['assets/textures/saturn_ring.png', '../images/fullsize/176saturn.jpg'], (tex) => {
      setupColorTexture(tex);
      tex.wrapS = THREE.ClampToEdgeWrapping;
      tex.wrapT = THREE.ClampToEdgeWrapping;
      ringMat.map = tex;
      ...
      ringGlowMat.needsUpdate = true;
    });

    loadWithFallback(['assets/textures/saturn_normal.jpg'], (tex) => {
      setupLinearTexture(tex);
      tex.wrapS = THREE.RepeatWrapping;
      tex.wrapT = THREE.ClampToEdgeWrapping;
      planetMat.normalMap = tex;
      planetMat.normalScale = new THREE.Vector2(0.28, 0.28);
      planetMat.needsUpdate = true;
    });
    ...
    const animateFrame = () => {
      ...
      // Very slow, smooth motion (as requested)
      // ~0.002–0.005 per frame @ 60fps
      planet.rotation.y += dt * 0.14;
      // ~0.0005–0.001 per frame @ 60fps
      ringGroup.rotation.z += dt * 0.04;
      saturn.position.y = Math.sin(t * 0.2) * 0.1;

      // Micro-parallax
      current.rx += (target.rx - current.rx) * 0.06;
      current.ry += (target.ry - current.ry) * 0.06;
      saturn.rotation.x = baseRot.x + current.rx;
      saturn.rotation.y = baseRot.y + current.ry;
      saturn.rotation.z = baseRot.z;

      // Subtle ring sparkle pulse + drift (cinematic, no jitter)
      const dustPulse = 0.5 + Math.sin(t * 0.9) * 0.5;
      ringDust.mat.opacity = 0.16 + dustPulse * 0.06;
      const sparklePulse = 0.5 + Math.sin(t * 1.3 + 1.7) * 0.5;
      ringSparkles.mat.opacity = 0.06 + sparklePulse * 0.05;
      ringDust.points.rotation.z += dt * 0.012;
      ringSparkles.points.rotation.z -= dt * 0.009;

      // Scroll-based camera zoom (hero only)
      const zoom = saturnScroll01 * 0.55;
      const pointerFactor = supportsHover && !prefersReducedMotion ? 1 : 0;
      camTarget.set(
        camBase.x + pointerNX * 0.45 * pointerFactor,
        camBase.y + pointerNY * 0.25 * pointerFactor,
        camBase.z - zoom
      );
      camCurrent.lerp(camTarget, 0.06);
      camera.position.copy(camCurrent);
      camera.lookAt(lookAt);
